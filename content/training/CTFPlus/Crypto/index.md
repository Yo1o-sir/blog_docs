...
既然\(gcd(e,\phi(n))=3\)，那么一定有\(gcd(e,p-1)=1\)或\(gcd(e,q-1)=1\)

我的意思是说，如果e与p-1互素，我们就把p-1当作新的\(\phi_n\),然后p可以看作新的模数，若是和q-1互素，也是相同道理

![exp](./index.assets/image-20260720163155042.png)

我测试过的，这里的e和p-1互素，简单说说这里的数学原理

在模N的乘法域中，进行计算特别不方便，但是如果我们知道它有两个素数因子，就能将这个乘法域拆成两个素数域，就是说模P或模Q域，将原来的RSA计算过程退化到P域上，发现e恰好可以与phi_p互素，所以说我们可以单纯考虑其中的一个素数域就能恢复出flag了
